...
class LightTransformerBlock(nn.Module):
    ...
    def __init__(self, dim,
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.conv = ProjectConvBlock(dim=dim, drop=drop)
    ...
    def forward(self, x):
        ...
        x = x.view(B * num_d * num_h * num_w, num_patches, C)
        # 第一个归一化和注意力
        x = x + self.drop_path(self.token_mixer(self.norm1(x)))
        # 第二个归一化和MLP
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
if "__main__" == __name__:
    x = torch.randn((2, 64, 64,64,8,32))
    att = LightTransformerBlock(dim=32,
            num_heads=8,  # 与原注意力头数一致
            mlp_ratio=4,  # 调整MLP扩展比例
            drop=0.1,  # 添加dropout正则化
            attn_drop=0.1)
    y = att(x)
    # print(y.shape)
